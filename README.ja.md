...
AIコーディングアシスタント（OpenAI Codex、VSCode Copilot、Google Antigravity、Gemini CLI）のプラグインを統合管理するRust製CLIツールです。Claude Code Pluginsをインポートして他の環境にデプロイできます。Skills、Agents、Prompts、Instructionsのダウンロード、インストール、同期をシームレスに行えます。
...
- **スコープ管理**: 個人レベル（`~/.codex/`、`~/.copilot/`、`~/.gemini/antigravity/`、`~/.gemini/skills/`）またはプロジェクトレベル（`.codex/`、`.github/`、`.gemini/skills/`）でインストール
...
| コマンド | 説明 | 詳細 |
|---------|------|------|
| `install` | プラグインをインストール | GitHub (`owner/repo`) またはマーケットプレイス (`plugin@market`) から |
| `list` | インストール済みプラグイン一覧 | `--json`, `--simple`, `--outdated` フラグに対応 |
| `info` | プラグインの詳細表示 | コンポーネント、作者、デプロイ状態を表示 |
| `enable` | プラグインを有効化 | キャッシュからターゲットにデプロイ |
| `disable` | プラグインを無効化 | ターゲットから削除、キャッシュは保持 |
| `uninstall` | プラグインを削除 | キャッシュを含め完全に削除 |
| `update` | プラグインを更新 | 利用可能な更新をチェック・適用 |

### 設定コマンド

| コマンド | 説明 | 詳細 |
|---------|------|------|
| `target` | ターゲット管理 | `list`, `add`, `remove` サブコマンド |
| `marketplace` | マーケットプレイス管理 | `list`, `add`, `remove`, `update` サブコマンド |

### プラグイン開発

| コマンド | 説明 | 詳細 |
|---------|------|------|
| `init` | テンプレート生成 | 新規プラグインのスケルトンを作成 |
...
|---------|------|------|
| `sync` | 環境間同期 | プラグインを同期 |
| `import` | Claude Code Pluginからインポート | 既存プラグインを変換・デプロイ |
| `managed` | TUIインターフェース | ビジュアルなプラグイン管理 |
