...
`extract_archive_with_source_path` は GitHub zipball 等をキャッシュへ展開する。zip-slip と symlink を防ぐため、`source_path` の有無に関わらず次を適用する。
...
5. zip エントリの unix mode が symlink（`S_IFLNK`）なら拒否する。
6. 最終ターゲットが展開先ディレクトリ配下であること。
7. 1件でも拒否したら `InvalidSource` で失敗する（fail-closed）。
