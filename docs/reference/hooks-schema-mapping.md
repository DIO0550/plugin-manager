...
> **要点:** フック実行コンテキスト（セッション ID / ツール名 / cwd / effort 等）を**環境変数**で渡すのは
> **Claude Code のみ**である。Codex / Antigravity / Copilot はコンテキストを **stdin の JSON** で渡し、
> 環境変数はプラグイン位置・CLI 設定などごく一部に限られる。

### Claude Code がフックに提供する変数（公式仕様・最新）

出典: https://code.claude.com/docs/en/hooks （公式本文取得済み、信頼度高）

| 変数名 | 利用可能イベント | 説明 |
|--------|----------------|------|
| `CLAUDE_PROJECT_DIR` | 全フック | プロジェクトルート。MCP/LSP サーバ環境にも設定される |
| `CLAUDE_PLUGIN_ROOT` | プラグインフック | プラグインのインストールディレクトリ（更新で変わりうる） |
| `CLAUDE_PLUGIN_DATA` | プラグインフック | プラグインの**永続データディレクトリ**（更新後も存続）★新規 |
| `CLAUDE_EFFORT` | ツール系イベント（`PreToolUse` / `PostToolUse` / `Stop` / `SubagentStop` 等） | 現ターンの effort レベル（`low` / `medium` / `high` / `xhigh` / `max`）★新規 |
| `CLAUDE_ENV_FILE` | `SessionStart` / `Setup` / `CwdChanged` / `FileChanged` | `export` 文を書き込むと以降の Bash へ変数を永続化できるファイルパス |
| `CLAUDE_CODE_REMOTE` | 全フック | リモート（Web）環境では `"true"`。ローカル CLI では未設定 |
| `CLAUDE_FILE_PATHS` ※ | （旧: ツール系イベント） | 操作対象ファイルパス。**最新の公式 hooks ドキュメントには記載がなく、旧情報の可能性あり**（削除はせず注記） |

> ※ `CLAUDE_FILE_PATHS` は本ドキュメントの旧版に記載があったが、2026-06-24 時点の公式 hooks
> ドキュメントには掲載されていない。廃止/改名された旧情報の可能性が高いため、利用前に最新の公式仕様で
> 確認すること。`CLAUDE_PLUGIN_DATA` / `CLAUDE_EFFORT` は今回追記した新規項目。

### Codex CLI がフックに提供する変数

出典: https://developers.openai.com/codex/hooks 、openai/codex `codex-rs/hooks/src/engine/discovery.rs`
（公式本文は 403 のため**公式ソースコードで直接確認**、信頼度高）

- フック実行コンテキストは **stdin の単一行 JSON のみ**で渡され、**環境変数では渡さない**。
- 以下4変数は**プラグイン由来のフックにのみ**設定される（通常のユーザ/プロジェクト hook には付かない）:

| 変数名 | 説明 |
|--------|------|
| `PLUGIN_ROOT` | インストール済みプラグインのルートディレクトリ |
| `PLUGIN_DATA` | プラグインの書き込み可能データディレクトリ |
| `CLAUDE_PLUGIN_ROOT` | `PLUGIN_ROOT` と同値（既存 Claude Code プラグイン互換のためのエイリアス） |
| `CLAUDE_PLUGIN_DATA` | `PLUGIN_DATA` と同値（同上、互換エイリアス） |

- 上記以外に Codex 固有のフック用環境変数は**存在しない**。コマンドは**セッションの cwd** を作業ディレクトリに実行される。
- `${PLUGIN_ROOT}` 等は command 文字列内で**プレースホルダ置換**も可能。

### Google Antigravity がフックに提供する変数

出典: https://antigravity.google/docs/hooks （本文 403 のため SDK README・複数の独立実装でクロスチェック）

- フック専用の環境変数は**存在しない**。むしろセキュリティ上、フック実行時に**環境変数はサニタイズ**される。
- 全コンテキストは **stdin の JSON（camelCase）** で渡る。
- フックの cwd は `~/.gemini/config`（CLI 版は `~/.gemini/antigravity-cli/...`）になるため、
  **プロジェクトディレクトリは環境変数ではなく stdin の `workspacePaths[0]`（先頭が canonical）から取得**する必要がある。

### Copilot CLI がフックに提供する変数

出典: GitHub Docs スニペット（公式本文は 403 のため一部未確認）

| 変数名 | 説明 | 性質 |
|--------|------|------|
| `COPILOT_HOME` | 設定ディレクトリのルート（未設定時 `~/.copilot`）。ユーザレベル hook は `$COPILOT_HOME/hooks/` | CLI 全体設定（**フック実行コンテキスト用ではない**） |
| `COPILOT_MODEL` | 使用中の AI モデル指定 | CLI 全体設定（同上） |

- `COPILOT_HOME` / `COPILOT_MODEL` はあくまで**設定用途**であり、ツール名や cwd 等のフック実行コンテキストは
  stdin の JSON で渡される。
- プラグイン由来の hook には「プロジェクト固有」「プラグイン固有」ディレクトリを示す**2つの環境変数が自動付与**される
  （`CLAUDE_PROJECT_DIR` / `CLAUDE_PLUGIN_ROOT` 相当と推測されるが、**正確な変数名は公式本文が 403 のため未確認**）。
- 加えて、フック定義（JSON, `version: 1`）の `"env"` オブジェクトでカスタム環境変数を注入可能:
...
| ツール | コンテキスト | フック用 env（コンテキスト系） | プラグイン位置 env |
|--------|--------------|------------------------------|--------------------|
| Claude Code | env + stdin JSON | `CLAUDE_PROJECT_DIR` / `CLAUDE_CODE_REMOTE` / `CLAUDE_EFFORT` / `CLAUDE_ENV_FILE` 等 | `CLAUDE_PLUGIN_ROOT` / `CLAUDE_PLUGIN_DATA` |
| Codex CLI | stdin JSON のみ | なし | `PLUGIN_ROOT` / `PLUGIN_DATA` + `CLAUDE_*` 互換エイリアス（プラグイン hook のみ） |
| Antigravity | stdin JSON のみ（env サニタイズ） | なし | なし（cwd=`~/.gemini/config`、project は `workspacePaths[0]`） |
| Copilot CLI | stdin JSON 主体 | `COPILOT_HOME` / `COPILOT_MODEL`（設定用）＋ 定義の `env` | プラグイン用2変数（名称未確認・プラグイン hook のみ） |
