...
> **⚠️ 未実装（将来仕様）**: このページに記載されている `~/.plm/config.toml` および `PLM_CONFIG` 環境変数は**現時点で未実装**です。読み込む処理は存在せず、ファイルを作成しても効果はありません。実装は [#333](https://github.com/DIO0550/plugin-manager/issues/333) で管理しています。現在実際に使用される設定ファイルは「[現在の実装状態](#現在の実装状態)」を参照してください。
...
- `PLM_HOME` はキャッシュルートの解決で参照されますが、全パスで一貫して尊重されるかは [#333](https://github.com/DIO0550/plugin-manager/issues/333) で確認中です。
