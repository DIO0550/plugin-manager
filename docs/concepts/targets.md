...
> ****Antigravity（IDE / CLI 共通）Hooks は公式 5 イベント対応の変換・配置を実装済み（[#309](https://github.com/DIO0550/plugin-manager/issues/309)）。単一 `hooks.json`・複数 Hook 同時配置は拒否（フルマージ未実装）。stdin/stdout ラッパーは未生成（インライン command）。
> *****CursorのHooksは単一の `hooks.json` に配置する。既存の非管理ファイルの上書きと、同一インストール内の複数 Hook コンポーネントは拒否する（フルマージは未実装）。
> ******Gemini CLI の Hooks は非対応。一般向けは Antigravity CLI へ移行済みで、Hooks は Antigravity（IDE / CLI 共通）仕様として [#309](https://github.com/DIO0550/plugin-manager/issues/309) で扱う（[#401](https://github.com/DIO0550/plugin-manager/issues/401)）。Enterprise 向け `gemini` ターゲットのレガシー化は [#402](https://github.com/DIO0550/plugin-manager/issues/402)。
...
- **Hooks 非対応**: Gemini CLI 単体の hooks 公式仕様は追わず、Antigravity（IDE / CLI）共通仕様へ一本化（[#401](https://github.com/DIO0550/plugin-manager/issues/401) / [#309](https://github.com/DIO0550/plugin-manager/issues/309)）。一般向け製品移行の詳細は [Discussion #27274](https://github.com/google-gemini/gemini-cli/discussions/27274)、ターゲット全体のレガシー化は [#402](https://github.com/DIO0550/plugin-manager/issues/402)
