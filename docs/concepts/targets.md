...
> ***CursorのInstructionsはProjectスコープ（`AGENTS.md`）のみ。Personalスコープの指示（User Rules）はアプリ設定画面で管理されるため対象外。Hooksは単一設定ファイル（`hooks.json`）のためマージ戦略の設計後に対応。
...
Claude Code側に対応イベントがないもの（`beforeShellExecution` 等のCursor固有イベント）は変換対象外。単一ファイルのため、複数プラグインのhooksを配置するには非管理エントリを壊さないマージ戦略が必要。

### コンポーネント配置場所（PLM 実装）

他ターゲット（Codex / Copilot）と同様、コンポーネント名は `flatten_name(plugin, original)` により `{plugin}_{original}` に平坦化して配置する（フラット 2 階層）。

| 種別 | ファイル形式 | Personal | Project |
|------|-------------|----------|---------|
| Skills | `SKILL.md` | `~/.cursor/skills/<flattened_name>/` | `.cursor/skills/<flattened_name>/` |
...
- **`AGENTS.md` は Codex ターゲットと同一ファイルを共有**: 両ターゲット有効時の所有権・削除の扱いは実装時に設計する
...
| Cursor 🚧 | `~/.cursor/` に配置（Skills / Agents / Commands 対応済み） | 不要（自動読み込み）。Hooksのみ `hooks.json` へのマージが必要 |
