...
> **⚠️ 未実装**: デフォルトスコープの設定機能（`~/.plm/config.toml` の `default_scope`）は未実装です（[#333](https://github.com/DIO0550/plugin-manager/issues/333)）。現在は `--scope` オプションで指定するか、未指定時の対話（TUI）選択を使用してください。

将来的に `~/.plm/config.toml` でデフォルトスコープを設定できるようにする予定です:
