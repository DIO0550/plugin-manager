...
fn new_uses_home_for_default_path() {
    ...
    let registry = TargetRegistry::new().unwrap();
    assert_eq!(
        registry.config_path,
        home.path().join(".plm").join("targets.json")
    );
}
...
fn new_errors_when_both_unset() {
    ...
    let err = TargetRegistry::new().unwrap_err();
    match err {
        crate::error::PlmError::TargetRegistry(msg) => {
            assert!(
        ...
        other => panic!("expected TargetRegistry error, got {:?}", other),
    }
}
